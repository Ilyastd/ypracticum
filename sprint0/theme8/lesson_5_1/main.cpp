...
int MatchDocument(const pair<int, vector<string>>& content, 
    ...
    int result = 0;
    for(const string& str: content.second) {
        result += query_words.count(str);
    }
    return result;
}
...
int main() {
    ...
    for (const auto& [document_id, rel] : FindDocuments(documents, stop_words, query)) {
        cout << "{ document_id = "s << document_id << ", relevance = "s << rel << " }"s << endl;
    }
}